...
for i in range(0,k+1):
  ...
  stateslistB.append('yLoc'+str(i))
  if i!=k:
    S_UpA.append('yLoc'+str(i+1))
  else:
    S_UpA.append('yLoc'+str(i))  
  if i!=0:
    S_DownA.append('yLoc'+str(i-1))
  else:
    S_DownA.append('yLoc'+str(i))

string = "(define (domain gws_2d_alt) \n \t (:requirements :strips :conditional-effects :negative-preconditions :disjunctive-preconditions) \n \t (:constants "
string2 = "(define (problem gws_2d_alt_pb) \n \t (:domain gws_2d_alt)  \n \t (:requirements :strips :conditional-effects :negative-preconditions :disjunctive-preconditions) \n \t     (:objects "

for x in stateslist+stateslistA+stateslistB:
    string += str(x)+" "
    string2 += str(x)+" "
string2 += ") \n \t   (:init (robot_S_position R) (robot_A_position Loc1)(robot_B_position yLoc1)) \n \t (:goal (or \n \t  "
string +=  ") \n \t (:predicates (robot_S_position ?x) (robot_A_position ?x) (robot_B_position ?x)(P0TRAN) )"

string2 += "(robot_S_position faraway) (robot_A_position Loc0)(robot_A_position Loc"+str(k)+")(robot_B_position yLoc0)(robot_B_position yLoc"+str(k)+"))))"
...
string +=  "))\n \t :effect (and"
for i,x in enumerate(stateslist):
  string +=  "\t \t (when (robot_S_position "+str(x)+") (and (P0TRAN) (robot_S_position "+str(E_Up[i])+" )  (not (robot_S_position "+str(x)+" )) ))"
for i,x in enumerate(stateslistB):
  if x != stateslistB[-2]:
    string +=  "\t \t (when (robot_B_position "+str(x)+") (and (P0TRAN)(robot_B_position "+str(S_UpA[i])+" )  (not (robot_B_position "+str(x)+" )) ))"
...
string +=  "))\n \t :effect (and"
for i,x in enumerate(stateslist):
  string +=  "\t \t (when (robot_S_position "+str(x)+") (and (P0TRAN) (robot_S_position "+str(E_Down[i])+" )  (not (robot_S_position "+str(x)+" )) ))"
for i,x in enumerate(stateslistB):
  if x != 'yLoc1':
    string +=  "\t \t (when (robot_B_position "+str(x)+") (and (P0TRAN)(robot_B_position "+str(S_DownA[i])+" )  (not (robot_B_position "+str(x)+" )) ))"
...
f = open('gws_2d_alt.pddl', 'w')
f.write(string)
f.close

f = open('gws_2d_alt_pb.pddl', 'w')
f.write(string2)
f.close